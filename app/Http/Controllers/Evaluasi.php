...
class Evaluasi extends Controller {
	...
	public function list(Request $request) {
		...
		$column_user = "uu.id AS uu__id,uu.nama_user AS uu__nama_user,uu.email_user AS uu__email_user,uu.whatsapp_user AS uu__whatsapp_user,uu.username_user AS uu__username_user,uu.jenis_kelamin AS uu__jenis_kelamin,uu.tanggal_lahir AS uu__tanggal_lahir,uu.agama_id AS uu__agama_id,uu.pekerjaan_id AS uu__pekerjaan_id,uu.penghasilan_id AS uu__penghasilan_id,uu.jenjang_id AS uu__jenjang_id";
		$data = DB::select("
			SELECT {$columns},{$column_user}
			FROM user_role ur
	...
	public function listCreatedAt(Request $request, $user_role_id) {
		...
		$new_data = array_map(function($row) use ($user_role_id) {
			$row->created_ats = DB::select("SELECT DISTINCT(created_at) FROM evaluasi_pengetahuan et WHERE et.deleted_at IS NULL AND user_role_id = :user_role_id",['user_role_id'=>$user_role_id]);
			return $row;
		},$data); 
