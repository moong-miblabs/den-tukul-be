...
class Login extends Controller {
	public function register(Request $request) {
    	...
    	$datas = DB::transaction(function () use ($datauser,$datarole) {
    		...
    		return DB::select("SELECT uu.id, nama_user, username_user, email_user, email_verified_at, whatsapp_user, whatsapp_verified_at FROM users uu WHERE uu.id = ?",[$datauser['id']]);
    	});
    	$data = $datas[0];
