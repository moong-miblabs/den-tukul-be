...
class AuthMiddleware {
    ...
    public function handle(Request $request, Closure $next): Response {
        ...
        if($decoded['error'] === FALSE){
            ...
            $users = DB::select("SELECT ur.id, user_id, role_id FROM user_role ur LEFT JOIN users uu ON ur.user_id = uu.id LEFT JOIN ms_role mr ON ur.role_id = mr.id where uu.id = ? AND uu.password_user = ? AND uu.deleted_at IS NULL limit 1",[$data_token->id,$data_token->password_user]);
            if(!empty($users)) {
